import { BadgeCheck, BadgeAlert, Trash, EyeOff, SquarePen, Save, Ban } from "lucide-react";
import useProducts from "../hooks/useProducts";
import { useState } from "react";

function TableProductList() {
	const { products, isLoading, error, deleteProduct, updateParcialProduct} = useProducts();
					...
					{products.map((product) => (
						...
						>
							...
							<td>
								{editingId === product._id ? (
									<input
										...
										onChange={(e) =>
											setEditValues({ ...editValues, desciption: e.target.value })
										}
									/>
								) : (
									product.desciption
							...
							<td className="px-4 py-3 border-b border-gray-700 flex gap-3">
								{editingId === product._id ? (
									<button className="text-red-400 transition" 
											onClick={() => {
											setEditingId(null);
											setEditValues({});
										}}
									...
									<button className="text-red-400 transition" onClick={() => deleteProduct(product._id)}>
									<Trash size={18} />
								</button>
								)}
